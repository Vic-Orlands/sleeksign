...
export async function PATCH(
  ...
  try {
    ...
    if (!group) {
      return NextResponse.json({ error: "Signer group not found" }, { status: 404 });
    }
    ...
    if (nextSignerIds) {
      ...
      if (hasUnknownSigner) {
        return NextResponse.json({ error: "One or more signers were not found" }, { status: 404 });
      }
    }
    ...
    return NextResponse.json({ error: "Failed to update signer group" }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to delete signer group" }, { status: 500 });
  }
}

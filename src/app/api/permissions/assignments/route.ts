...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!workspaceId || !memberId || !roleId) {
      return NextResponse.json({ error: "workspaceId, memberId, and roleId are required" }, { status: 400 });
    }
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!workspaceId || !assignmentId) {
      return NextResponse.json({ error: "workspaceId and assignmentId are required" }, { status: 400 });
    }

<script lang="ts">
	...
	import { areRoleConfigsEqual, type RoleConfig, type WorkflowMode } from "$lib/field-utils";
	...
	type SignerGroupRecord = { id: string; name: string; signers: DirectorySigner[] };
	...
	const privateRoles = $derived(roleConfigs.filter((role) => role.scope === "private"));
	...
	$effect(() => {
		...
		void (async () => {
			try {
				const res = await fetch(`/api/teams?workspaceId=${encodeURIComponent(ws)}&summary=1`);
				const data = await res.json();
		...
		void (async () => {
			try {
				...
				const data = await res.json();
				if (!cancelled) directorySigners = Array.isArray(data?.signers) ? data.signers : [];
			} catch {
		...
		void (async () => {
			try {
				const res = await fetch(`/api/signer-groups?workspaceId=${encodeURIComponent(ws)}`);
				const data = await res.json();
				if (!cancelled) signerGroups = Array.isArray(data?.groups) ? data.groups : [];
			} catch {
	...
	async function parseCsvPreview() {
		...
		try {
			const res = await fetch("/api/bulk-send/parse", { method: "POST", body: formData });
			const data = await res.json();
			if (!res.ok) throw new Error(data.error || "Unable to parse CSV");
			csvPreview = Array.isArray(data.preview?.recipients) ? data.preview.recipients : [];
	...
	async function ensurePacket(mode: WorkflowMode) {
		...
		const existingPacket = packets.find(
			(packet) =>
				packet.mode === mode && areRoleConfigsEqual(packet.roleConfigs, roleConfigs),
		);
	...
	async function sendDocumentNow() {
		...
		const targets =
			sendPath === "email"
				...
				: sendPath === "signer"
					? selectedSignerId
						? [{ kind: "signer" as const, signerId: selectedSignerId, roleName }]
						: []
					: selectedGroupId
...
{#if !doc}
	<aside class="flex h-full min-h-[320px] min-w-0 items-center justify-center bg-card p-4">
		<div
...
{:else}
	<aside class="grid h-full min-w-0 grid-rows-[auto_minmax(0,1fr)_auto] overflow-x-hidden bg-card">
		<div class="px-5 pb-3 pt-5">
			<div class="flex items-start justify-between gap-3">
				<div class="min-w-0">
					<h2 class="truncate text-xs font-semibold uppercase tracking-widest">{doc.name}</h2>
		...
		<div class="min-h-0 overflow-auto px-5 pb-5">
			<div class="flex min-w-0 flex-col gap-4">
				<div class="grid w-full min-w-0 grid-cols-2 gap-1 rounded-lg bg-muted/50 p-1 sm:grid-cols-4">
					{#each [
				...
				{#if activeTab === "overview" && counts}
					<div class="flex flex-col gap-6 pt-1">
						<div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
							<div class="rounded-3xl bg-background p-3 shadow-sm">
								<p class="font-mono text-[9px] uppercase tracking-widest text-muted-foreground">
									Fields
								</p>
								<p class="mt-2 truncate font-mono text-sm text-foreground">{counts.fields}</p>
							</div>
							<div class="rounded-3xl bg-background p-3 shadow-sm">
								<p class="font-mono text-[9px] uppercase tracking-widest text-muted-foreground">
									File type
						...
						<div class="grid gap-4 rounded-3xl bg-muted/35 p-4">
							<div class="flex items-center gap-2">
								<ShieldCheckIcon class="size-4 text-orange-500" />
								<p class="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
									Overview
								</p>
							</div>
							<div class="grid gap-3 sm:grid-cols-2">
								<div class="rounded-2xl bg-background px-3 py-3">
									<p class="font-mono text-[9px] uppercase tracking-widest text-muted-foreground">
										Team
									</p>
									<p class="mt-1 text-sm font-medium text-foreground">
										{teams.find((team) => team.id === selectedTeamId)?.name || "Unassigned"}
									</p>
								</div>
								<div class="rounded-2xl bg-background px-3 py-3">
									<p class="font-mono text-[9px] uppercase tracking-widest text-muted-foreground">
							...
							<label class="grid gap-2 text-sm">
								...
								<select
									...
									class="rounded-2xl bg-background px-3 py-2.5"
								>
							...
							<label
								class="flex items-center justify-between gap-3 rounded-2xl bg-background px-4 py-3 text-sm"
							>
								<div>
									<p class="font-medium">Require email OTP before viewing</p>
						...
						<div class="rounded-3xl bg-background/80 p-4 shadow-sm">
							<div class="flex items-start gap-3">
								<span
									class="flex size-9 items-center justify-center rounded-2xl bg-secondary text-muted-foreground"
								>
									<PaperPlaneTiltIcon class="size-4" />
								</span>
								<div class="min-w-0">
									<h3 class="font-mono text-[10px] font-semibold uppercase tracking-widest">
										Ready to share
									</h3>
									<p class="mt-2 text-sm leading-6 text-muted-foreground">
										Send this link after the field layout is ready. Signers will enter their
										details, complete required fields, and generate the signed PDF.
									</p>
								</div>
							</div>
							{#if !canShare}
								<p class="mt-3 font-mono text-[10px] uppercase tracking-widest text-destructive">
				...
				{:else if activeTab === "link"}
					<div class="flex flex-col gap-4">
						...
						<div class="rounded-3xl bg-muted/35 p-4">
							<p class="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
								{selectedModeOption.mode}
							</p>
							<p class="mt-1 text-sm font-medium text-foreground">{selectedModeOption.title}</p>
							<p class="mt-1 text-sm text-muted-foreground">{selectedModeOption.copy}</p>
						</div>
								...
								{#each getPacketRoleLinks(selectedPacket) as { role, url } (`${selectedPacket.id}-${role.name}`)}
									...
									>
										<div class="min-w-0">
											...
											<p class="mt-1 truncate font-mono text-xs text-foreground">{url}</p>
										</div>
										<Button
											variant="outline"
				...
				{:else if activeTab === "bulk"}
					<div class="flex flex-col gap-4">
						<div class="rounded-3xl bg-background p-4 shadow-sm">
							<p class="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
								Send document
							</p>
								...
								{:else if sendPath === "signer"}
									...
									>
										...
										{#each directorySigners as signer (signer.id)}
											<option value={signer.id}>{signer.name} · {signer.email}</option>
										{/each}
									</select>
								{:else}
									<select
							...
							<div class="mt-3 grid gap-3">
								...
								>
									...
									{#if csvFile}
										...
										<span class="text-xs text-muted-foreground">Click to choose a different CSV</span>
									{:else}
										<span class="text-sm font-medium">Click to upload a CSV file</span>
								...
								<input
									...
									accept=".csv,text/csv"
									class="sr-only"
									onchange={(event) => (csvFile = event.currentTarget.files?.[0] || null)}
								/>
								...
								<div class="mt-3 grid gap-2">
									{#each csvPreview.slice(0, 5) as row, index (`${String(row.signerEmail || index)}-${index}`)}
										<div class="rounded-2xl bg-muted/35 px-3 py-2 text-sm">
											{String(row.signerName || "Recipient")} · {String(row.roleName || "Role")} ·
						...
						<div class="rounded-3xl bg-background p-4 shadow-sm">
							...
							<div class="mt-3 grid gap-2">
								{#if jobs.length === 0}
									<p class="text-sm text-muted-foreground">No bulk send jobs yet.</p>
								{:else}
									{#each jobs as job (job.id)}
										<div class="rounded-2xl bg-muted/35 p-3">
											...
											<p class="mt-2 text-sm">
												{job.createdCount}/{job.totalRows} created · {job.sentCount} sent ·
				...
				{:else if activeTab === "activity"}
					<div>
						<h3 class="mb-4 font-mono text-[10px] font-semibold uppercase tracking-widest">
							Audit trail
						</h3>

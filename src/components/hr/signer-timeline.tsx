    ...
    <div className="flex flex-col gap-4">
      {sessions.map((session, index) => (
        <div key={session.id} className="relative flex gap-3">
          <div className="flex flex-col items-center">
            ...
            {index < sessions.length - 1 ? <span className="h-10 w-px bg-border" /> : null}
          </div>
          <div className="min-w-0 flex-1 pb-2">
            <div className="flex items-center justify-between gap-2">
              <p className="truncate text-sm font-medium">{session.signerName || "Anonymous signer"}</p>
              <StatusBadge status={session.status} />
            </div>
            <p className="mt-1 flex items-center gap-1 text-xs text-muted-foreground">
              <MailIcon className="size-3" />
              {session.signerEmail || "No email"} sent {format(new Date(session.createdAt), "PPp")}
            </p>
          </div>
        </div>
      ))}
      <div className="flex gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">Waiting for all signers to complete</p>
        </div>
      </div>
    </div>
  )

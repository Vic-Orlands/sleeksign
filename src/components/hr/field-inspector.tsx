  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        ...
        <p className="mt-1 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Page {selectedField.page + 1}</p>
      </div>
      <label className="flex flex-col gap-1.5">
        <span className="font-mono text-[10px] font-medium uppercase tracking-widest text-muted-foreground">Assigned to</span>
      ...
      <div className="grid grid-cols-2 gap-3">
        {(["x", "y", "width", "height"] as const).map((key) => (
          <label key={key} className="flex flex-col gap-1.5">
            <span className="font-mono text-[10px] font-medium uppercase tracking-widest text-muted-foreground">{key} (%)</span>
            <Input
              type="number"
      ...
      <label className="flex flex-col gap-1.5">
        <span className="font-mono text-[10px] font-medium uppercase tracking-widest text-muted-foreground">Rotation</span>
        <Input value="0°" readOnly className="font-mono" />
      </label>
      <Separator />
      <Button variant="destructive" className="w-full" onClick={() => onDelete(selectedField.id)}>

...
async function loadWorkspaceAccessSnapshot(workspaceId: string) {
  ...
  const [roleData, memberData, invitationData] = await Promise.all([
    fetchWorkspaceRequest<{ role?: string }>("/api/auth/organization/get-active-member-role"),
    fetchWorkspaceRequest<{ members: WorkspaceMember[] }>(`/api/auth/organization/list-members?organizationId=${encodeURIComponent(workspaceId)}`),
    fetchWorkspaceRequest<WorkspaceInvitation[]>(`/api/auth/organization/list-invitations?organizationId=${encodeURIComponent(workspaceId)}`),
  ])
...
function AccountMenu({ collapsed }: { collapsed: boolean }) {
  ...
  const [confirmDeleteWorkspaceOpen, setConfirmDeleteWorkspaceOpen] = React.useState(false)
  ...
  const workspaceItems = React.useMemo(() => {
    ...
    const created = optimisticCreatedWorkspaces.filter((item) => !deletedIds.has(item.id) && !existingIds.has(item.id))

    return [...base, ...created]
  }, [optimisticCreatedWorkspaces, optimisticDeletedWorkspaceIds, optimisticWorkspaceNames, workspaces])
  const activeWorkspace = workspaceItems.find((item) => item.id === selectedWorkspaceId) || null
  ...
  const inviteRoleOptions = canDeleteWorkspace ? ["member", "admin", "owner"] : ["member", "admin"]
  ...
  async function deleteWorkspace(workspaceId: string) {
    const currentWorkspace = workspaceItems.find((item) => item.id === workspaceId)
    const fallbackWorkspace = workspaceItems.find((item) => item.id !== workspaceId)?.id || ""

    try {
      setWorkspaceBusy("deleting")
      setOptimisticDeletedWorkspaceIds((current) => [...new Set([...current, workspaceId])])
      ...
      if (currentWorkspace) {
        setOptimisticDeletedWorkspaceIds((current) => current.filter((item) => item !== currentWorkspace.id))
      }
  ...
  async function updateMemberRole(member: WorkspaceMember, role: WorkspaceRole) {
    ...
    try {
      ...
      toast.success(role === "owner" ? "Ownership transferred" : "Member role updated")
      await loadWorkspaceData(selectedWorkspaceId)
    } catch {
      setMembers(snapshot)
  ...
  return (
    <div ref={menuRef} className="relative">
      ...
      <Dialog open={confirmDeleteWorkspaceOpen} onOpenChange={setConfirmDeleteWorkspaceOpen}>
        <DialogContent className="rounded-none border-border bg-popover shadow-sm">
          <DialogHeader>
            <DialogTitle className="font-mono text-xs uppercase tracking-widest">Delete workspace?</DialogTitle>
            <DialogDescription>
              This removes {activeWorkspace?.name || "this workspace"} and its team access from your workspace list.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="rounded-none border-border">
            <Button variant="outline" onClick={() => setConfirmDeleteWorkspaceOpen(false)}>
              Cancel
            </Button>
            <Button
              variant="destructive"
              disabled={!selectedWorkspaceId || workspaceBusy === "deleting"}
              onClick={() => selectedWorkspaceId && deleteWorkspace(selectedWorkspaceId)}
            >
              Delete Workspace
            </Button>
          </DialogFooter>
          ...
          <div className="grid max-h-[82vh] overflow-hidden sm:grid-cols-[220px_minmax(0,1fr)]">
            ...
            <div className="overflow-auto p-5">
              ...
              <section className="mt-4 border border-border bg-background p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">The active workspace controls which documents, members, and invitations you can manage.</p>
                  </div>
                  {activeWorkspace ? <Badge variant="outline">{activeRole}</Badge> : null}
                </div>
                ...
                {activeWorkspace ? (
                  <form className="mt-4 grid gap-3 border border-border bg-secondary p-3" onSubmit={renameWorkspace}>
                    <label className="grid gap-1.5">
                      <span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Active Workspace</span>
                      <Input
                        key={selectedWorkspaceId}
                        name="workspaceName"
                        defaultValue={activeWorkspace.name}
                        placeholder="Workspace name"
                        disabled={!canManageWorkspace || workspaceBusy === "renaming"}
                      />
                    </label>
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <p className="text-xs text-muted-foreground">
                        {canManageWorkspace ? "Owners and admins can rename this workspace." : "Only owners and admins can rename this workspace."}
                      </p>
                      <div className="flex items-center gap-2">
                        <Button type="submit" size="sm" disabled={!canManageWorkspace || workspaceBusy === "renaming"}>
                          Save Workspace
                        </Button>
                        <Button
                          type="button"
              ...
              <section className="mt-4 border border-border bg-background p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Workspace Members</p>
                    <p className="mt-1 text-sm text-muted-foreground">Owners can transfer ownership. Admins can manage members. Members can review access.</p>
                  </div>
                  {workspaceDataLoading ? <Badge variant="secondary">Syncing</Badge> : null}
                </div>

                <form className="mt-3 grid gap-2 border border-border bg-secondary p-3 sm:grid-cols-[minmax(0,1fr)_120px_auto]" onSubmit={inviteMember}>
                  <Input
                    value={inviteEmail}
                  ...
                  <Button type="submit" disabled={!canManageWorkspace || !selectedWorkspaceId}>
                    Invite
                  </Button>
                </form>
                ...
                <div className="mt-3 grid gap-2">
                  {members.length === 0 ? (
                    ...
                    members.map((member) => (
                      <div key={member.id} className="grid gap-3 border border-border px-3 py-3 sm:grid-cols-[minmax(0,1fr)_110px_auto] sm:items-center">
                        <div className="min-w-0">
                          <p className="truncate text-sm font-medium">{member.user.name}</p>
                          <p className="truncate text-xs text-muted-foreground">{member.user.email}</p>
                        </div>
                        <Badge variant="outline" className="justify-center">{member.role}</Badge>
                          ...
                          {canManageMember(member) ? (
                            <>
                              ...
                              >
                                ...
                                {canDeleteWorkspace ? <option value="owner">owner</option> : null}
                              </Select>
                              <Button
                                variant="ghost"
                                size="sm"
              ...
              <section className="mt-4 border border-border bg-background p-4">
                <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Pending Invitations</p>
                <div className="mt-3 grid gap-2">
                  {invitations.length === 0 ? (
                    ...
                    invitations.map((invitation) => (
                      <div key={invitation.id} className="grid gap-2 border border-border px-3 py-3 sm:grid-cols-[minmax(0,1fr)_100px_auto] sm:items-center">
                        <div className="min-w-0">
                          <p className="truncate text-sm font-medium">{invitation.email}</p>
                          <p className="text-xs text-muted-foreground">Status: {invitation.status}</p>
                        </div>
                        <Badge variant="outline" className="justify-center">{invitation.role}</Badge>
                        <div className="flex justify-end">
                          <Button
                            variant="ghost"
                            size="sm"
                            disabled={!canManageWorkspace || invitationBusyId === invitation.id}
    ...
    >
      ...
      <form className="grid gap-2 border-t border-border pt-2" onSubmit={onCreateWorkspace}>
        <Input
          ...
          className="h-8"
        />
        <Button size="sm" className="justify-start gap-2" type="submit" disabled={creating}>

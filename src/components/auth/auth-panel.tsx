...
const authContent: Record<AuthMode, { eyebrow: string; title: string; copy: string }> = {
  signin: {
...
function AuthPanel({ mode, token, nextPath }: AuthPanelProps) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const name = String(formData.get("name") || email.split("@")[0] || "SleekSign User")
      ...
      if (isSignUp) {
        ...
        if (workspaceName) {
          ...
          if (organization?.data?.id) {
            localStorage.setItem("sleeksign:workspace-id", organization.data.id)
          }
        }
      } else {
    ...
    <LayoutGroup id="auth-flow">
      <main className="grid min-h-svh bg-[var(--paper)] text-foreground lg:grid-cols-[minmax(0,0.95fr)_minmax(480px,1.05fr)]">
        <section className="hidden border-r border-border bg-background p-8 lg:flex lg:flex-col">
          ...
          <div className="mt-auto max-w-xl">
            <p className="font-mono text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">Document Operations</p>
            <h1 className="mt-4 text-4xl font-semibold tracking-normal text-foreground">Prepare, send, sign, and review HR documents from one focused workspace.</h1>
            <div className="mt-8 grid gap-3">
              <AuthSignal icon={FileTextIcon} label="Edited documents stay separate from untouched uploads" />
              <AuthSignal icon={CheckCircle2Icon} label="Signed documents are reviewable before download" />
              <AuthSignal icon={Building2Icon} label="Workspace switching keeps teams in context" />
            </div>
          </div>
        </section>
      ...
      <section className="flex min-h-svh items-center justify-center px-4 py-8 sm:px-6">
        ...
        >
          ...
          <motion.div layout className="mt-8 sm:mt-0" transition={springTransition}>
            <motion.p layoutId="auth-eyebrow" className="font-mono text-[10px] font-semibold uppercase tracking-widest text-muted-foreground" transition={springTransition}>
              {content.eyebrow}
            </motion.p>
            <motion.h2 layoutId="auth-title" className="mt-3 text-2xl font-semibold tracking-normal" transition={springTransition}>
              {content.title}
            </motion.h2>
            <motion.p layoutId="auth-copy" className="mt-2 text-sm text-muted-foreground" transition={springTransition}>
              {content.copy}
            </motion.p>
          </motion.div>

          <motion.form layout className="mt-6 flex flex-col gap-3" onSubmit={submit} transition={springTransition}>
            ...
            {!isReset ? (
              <AuthField layoutId="auth-field-email" icon={MailIcon} label="Email" type="email" name="email" autoComplete="email" placeholder="alex@company.com" />
            ) : null}
            <AnimatePresence initial={false} presenceAffectsLayout={false}>
              {isSignIn || isSignUp ? (
                <AuthField
            ...
            <motion.div layoutId="auth-submit" className="mt-2" transition={springTransition}>
          ...
          {isSignIn || isSignUp ? (
            <>
              <motion.div layoutId="auth-divider" className="my-5 grid grid-cols-[1fr_auto_1fr] items-center gap-3" transition={springTransition}>
                <span className="h-px bg-border" />
                <span className="font-mono text-[9px] uppercase tracking-widest text-muted-foreground">or</span>
                <span className="h-px bg-border" />
              </motion.div>

              <motion.div layoutId="auth-google" transition={springTransition}>
                <Button variant="outline" className="w-full gap-2" onClick={signInWithGoogle}>
                  <span data-icon="inline-start" className="text-[12px] font-bold normal-case tracking-normal">G</span>
          ...
          <motion.div layoutId="auth-switcher" className="mt-5 flex items-center justify-between gap-3 border-t border-border pt-4 text-sm text-muted-foreground" transition={springTransition}>
...
function AuthField({
  ...
  return (
    <motion.label
      layout
      layoutId={layoutId}
      initial={{ opacity: 0, height: 0, y: -6, filter: "blur(2px)", overflow: "hidden" }}
      animate={{ opacity: 1, height: "auto", y: 0, filter: "blur(0px)", overflow: "visible" }}
      exit={{ opacity: 0, height: 0, y: -6, filter: "blur(2px)", overflow: "hidden" }}
...
function AuthSignal({ icon: Icon, label }: { icon: React.ElementType; label: string }) {
  return (

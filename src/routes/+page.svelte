<script lang="ts">
	...
	onMount(() => {
		...
		if (browser) {
			void import("$lib/components/marketing/packet-model-showcase.svelte").then((mod) => {
...
<main class="relative min-h-screen overflow-hidden bg-(--paper) font-sans text-foreground">
	<!-- Hero Section -->
	<section class="relative z-10 mx-auto w-[90%] max-w-7xl pb-32 pt-28">
		<div
		...
		>
			<div
				class="mb-16 flex flex-col items-center justify-center transition-opacity duration-800 delay-200"
				class:opacity-0={!heroReady}
				class:opacity-100={heroReady}
			>
				<span class="font-cursive text-[48px] leading-none text-foreground sm:text-[64px]">
					SleekSign
				</span>
				<span
					class="mt-3 text-[10px] font-medium uppercase tracking-[0.2em] text-muted-foreground"
				>
	...
	<section class="relative z-10 mb-20 flex w-full justify-center py-12 lg:py-18">
		<div class="grid w-fit grid-cols-1 items-center lg:grid-cols-2">
					...
					{#each metrics as metric (metric.label)}
						...
						>
							<div class="ml-auto flex items-center gap-2 opacity-80">
								...
								<p class="text-[10px] font-medium uppercase tracking-[0.15em] text-foreground">
									{metric.label}
								</p>
							</div>
							<p
			...
			>
				<p class="mb-8 text-[9px] font-medium uppercase tracking-[0.2em] text-orange-500/70">
					Use cases
				</p>
				<ul class="list-none space-y-5 text-[14px] font-light leading-relaxed text-foreground/80 sm:text-[15px]">
	...
	<section class="relative z-10 mt-10 w-full overflow-hidden py-24">
		...
		>
			<p class="mb-4 text-[9px] font-medium uppercase tracking-[0.2em] text-orange-500/70">
				Platform Interface
			</p>
			<h2
		...
		>
			<div class="flex w-full flex-col gap-6 md:w-1/3">
				{#each galleryItems as item, i (item.src)}
					...
					>
						<div class="mb-3 flex items-center gap-2 text-[10px] font-medium uppercase tracking-[0.2em] text-foreground">
							{item.label}
							{#if activeGalleryTab === i}
								<span in:fly={{ x: -5, duration: 300 }} out:fade={{ duration: 150 }}>
	...
	<!-- Final Section -->
	<section class="relative z-10 mx-auto flex w-[85%] max-w-5xl flex-col items-center py-24 text-center">
		<div
		...
		>
			<p class="mb-6 text-[9px] font-medium uppercase tracking-[0.2em] text-orange-500/70">
			...
			>
				...
				<span class="absolute -right-4 bottom-0 font-cursive text-4xl">&quot;</span>
			</p>
		</div>
	</section>
</main>

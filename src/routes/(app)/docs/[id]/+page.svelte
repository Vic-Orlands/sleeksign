<script lang="ts">
  ...
  const serverDocument = $derived(
    (data.document as DocumentRecord | null) ?? null,
  );
  ...
  const canShare = $derived(
    Boolean(document?.fields?.length) && unassignedCount === 0,
  );

  const isDocumentUploading = $derived(
    Boolean(document) &&
      (!document?.fileUrl || document.uploadStatus === "pending_upload"),
  );
...
<div class="grid h-full grid-rows-[auto_minmax(0,1fr)] bg-(--paper)">
  ...
  >
    <div class="flex min-w-0 items-center gap-3">
      <Button variant="ghost" size="sm" onclick={() => goto("/docs")}
        >← Documents</Button
      >
      <div class="min-w-0">
    ...
    {#if document}
      <div class="flex shrink-0 items-center gap-2">
        ...
        <Button
          ...
          disabled={isDocumentUploading}
        >
          <PaperPlaneTiltIcon class="size-3" />
          Share Document
        </Button>
      </div>
    {/if}
  ...
  <div class="min-h-0 p-3 sm:p-4">
    {#if loadError && !document}
      ...
      >
        <p>{loadError}</p>
        <Button variant="outline" onclick={() => goto("/docs")}
...
{#if document}
  ...
  >
    <span id="review-sheet-title" class="sr-only"
      >Review document before sharing</span
    >
    <div class="h-full min-h-0 w-full">

<script lang="ts">
	...
	const activeUpload = $derived(activeUploadId ? uploads[activeUploadId] : undefined);
...
{#if activeUpload && activeUpload.status === "uploading"}
	...
	>
		<div class="w-[min(92vw,24rem)] rounded-lg border border-border bg-background px-5 py-4">
			...
			>
				<span class="min-w-0 truncate text-sm font-medium">{activeUpload.name}</span>
			</div>
...
>
	<DialogContent>
		<DialogHeader>
			...
			<DialogDescription>
				This will remove the document from document management while keeping signer history
				available in the other views.
			</DialogDescription>
		</DialogHeader>
		...
		<DialogFooter>
			...
			>
				...
				<Button type="submit" variant="destructive" loading={documentAction === "delete"}>
					Delete
				</Button>
			</form>
		</DialogFooter>
...
>
	<DialogContent>
		...
		<DialogFooter>
			...
			>
				<input type="hidden" name="documentId" value={documentToArchive?.id || ""} />
				<Button type="submit" loading={documentAction === "archive"}>Archive</Button>
			</form>
		</DialogFooter>
	</DialogContent>
</Dialog>
...
>
	<DialogContent>
		...
		<DialogFooter>
			...
			>
				<input type="hidden" name="documentId" value={documentToRestore?.id || ""} />
				<Button type="submit" loading={documentAction === "restore"}>Restore</Button>
			</form>
		</DialogFooter>
	</DialogContent>
</Dialog>

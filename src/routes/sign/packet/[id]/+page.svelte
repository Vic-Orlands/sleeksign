<script lang="ts">
	...
	async function updateValue(fieldId: string, value: string) {
		...
		await fetch(`/api/public-packets/${packetId}`, {
			method: "PATCH",
			body: JSON.stringify({ roleName, copyId: copyId || null, fieldId, value }),
		});
	...
	async function handleFieldClick(field: Field) {
		...
		if (field.type === "checkbox") {
			await updateValue(field.id, currentValues[field.id] === "true" ? "false" : "true");
			return;
		}
		selectedField = field;
		isMakerOpen = true;
...
{:else if loadError || !context}
	<div class="flex h-screen items-center justify-center bg-background">{loadError || "Document not found"}</div>
{:else if finalPdfUrl}
	<div class="flex min-h-screen items-center justify-center bg-(--paper) p-6">
...
{:else}
	<div class="flex h-screen flex-col bg-(--paper)">
		<header class="flex h-14 items-center justify-between border-b border-border bg-background px-5">
			<div class="min-w-0">
			...
			<Button disabled={!allFieldsSigned} loading={isFinalizing} onclick={finalize}>
				Complete
			</Button>
		</header>
		<section class="sleek-grid min-h-0 flex-1 overflow-auto bg-zinc-100 px-6 py-8">
			<PdfCanvasViewer fileUrl={context.document.fileUrl} class="mx-auto w-full max-w-[840px]" pageClassName="relative bg-white">
				{#snippet renderOverlay(pageIndex, metrics)}
					{#each fields.filter((field) => field.page === pageIndex) as field (field.id)}
						...
						<button
							type="button"
							class="absolute flex items-center justify-center border {complete ? 'border-emerald-600 bg-emerald-50/95' : 'border-blue-500/70 bg-blue-50/95'}"
						...
						>
							...
							{:else}
								<span class="font-mono text-[10px] uppercase">{field.type}</span>
							{/if}
						</button>
					{/each}
				{/snippet}
